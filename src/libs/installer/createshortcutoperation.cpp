...
bool CreateShortcutOperation::performOperation()
{
    ...
    QString workingDir = takeArgument(QString::fromLatin1("workingDirectory="), &args);
    const QString description = takeArgument(QString::fromLatin1("description="), &args);
    ...
    const bool linked = createLink(linkTarget, linkLocation, workingDir, targetArguments, iconPath, iconId, description);
    if (!linked) {
        setError(UserDefinedError);
